...
import {
  SALES_ORDER_GROUP_BY_OPTIONS,
  salesDeliveryStatusLabel,
...
export function SalesOrdersView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const lockedOnly = activeFilterId === "locked";
    const cancelled = activeFilterId === "cancelled";
    const deliveryStatus =
      activeFilterId === "waiting" ||
      activeFilterId === "ready" ||
      activeFilterId === "delivered"
        ? activeFilterId
        : "all";
    ...
    const res = await getSalesQuotationsList({
      search: searchQuery,
      ordersOnly: true,
      status: cancelled ? "cancelled" : "all",
      lockedOnly,
      deliveryStatus,
      sortBy,
      sortDir,
  ...
  const grouped = useMemo(() => {
    ...
    for (const row of rows) {
      let key = "Other";
      if (groupBy === "delivery")
        key = salesDeliveryStatusLabel(row.delivery_status);
  ...
  function handleExport() {
    ...
    const lines = rows.map((r) =>
      [
        ...
        salesDeliveryStatusLabel(r.delivery_status),
        r.is_locked ? "Yes" : "No",
        r.organization_name || "",
      ]
    ...
    toast.success("Exported sales orders CSV");
  }
  ...
  const kanbanColumns: {
    id: "waiting" | "ready" | "delivered" | "cancelled";
    label: string;
  }[] = [
          ...
          <TableBody>
            {items.map((row) => (
              ...
              >
                ...
                <TableCell>
                  <span
                    className={`inline-flex items-center rounded-sm border px-2 py-0.5 text-[11px] font-medium ${deliveryBadgeClass(row.delivery_status)}`}
                  >
                    {salesDeliveryStatusLabel(row.delivery_status)}
                  </span>
                </TableCell>
                <TableCell>
                  <span className="inline-flex items-center rounded-sm border px-2 py-0.5 text-[11px] font-medium bg-emerald-100 text-emerald-800 border-emerald-200">
          ...
          {viewMode === "list" ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-48">
                ...
                >
                  {SALES_ORDER_GROUP_BY_OPTIONS.map((opt) => (
                    <DropdownMenuRadioItem
                      key={opt.id}
                      value={opt.id}
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden min-h-[320px]">
        {loading ? (
          ...
          <SalesEmptyState
            ...
            description="Confirm a quotation to create a sales order, or adjust search and filters."
            action={
              <Button
                size="sm"
            ...
            {kanbanColumns.map((col) => {
              const cards =
                col.id === "cancelled"
                  ...
                  : rows.filter(
                      (r) =>
                        r.status !== "cancelled" && r.delivery_status === col.id
                    );
              return (
                <div
                  key={col.id}
