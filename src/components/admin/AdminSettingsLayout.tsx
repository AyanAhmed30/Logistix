...
const USER_PROFILE_HASH_PREFIX = "#user-profile/";
const USER_CREATE_EVENT = "logistix:user-create";

function isUserFormHash(hash: string) {
  return hash === USER_CREATE_HASH || hash.startsWith(USER_PROFILE_HASH_PREFIX);
}
...
export function AdminSettingsLayout({
  ...
  const [isFullPageForm, setIsFullPageForm] = useState(false);
  ...
  useEffect(() => {
    function syncCreateMode() {
      ...
      const orgOpen = activeTab === "organization" && hash === ORG_CREATE_HASH;
      const userOpen = activeTab === "create" && isUserFormHash(hash);
      setIsFullPageForm(orgOpen || userOpen);
    }
    function onOrgCreateEvent(event: Event) {
      const detail = (event as CustomEvent<{ open?: boolean }>).detail;
      if (activeTab !== "organization") return;
      if (typeof detail?.open === "boolean") {
        setIsFullPageForm(detail.open);
        return;
      }
      syncCreateMode();
    }
    function onUserCreateEvent(event: Event) {
      const detail = (event as CustomEvent<{ open?: boolean }>).detail;
    ...
    window.addEventListener(ORG_CREATE_EVENT, onOrgCreateEvent);
    window.addEventListener(USER_CREATE_EVENT, onUserCreateEvent);
    return () => {
      window.removeEventListener("hashchange", syncCreateMode);
      window.removeEventListener("popstate", syncCreateMode);
      window.removeEventListener(ORG_CREATE_EVENT, onOrgCreateEvent);
      window.removeEventListener(USER_CREATE_EVENT, onUserCreateEvent);
    };
  }, [activeTab]);

  function handleNavClick(tab: AdminTab) {
    const hash = window.location.hash;
    if (hash === ORG_CREATE_HASH || isUserFormHash(hash)) {
  ...
  return (
    <div className={isFullPageForm ? "min-h-[70vh]" : "space-y-5"}>
      {!isFullPageForm ? (
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:gap-0">
        ...
        <main className={`min-w-0 flex-1 ${isFullPageForm ? "" : "lg:pl-6 xl:pl-8"}`}>
          <div className={isFullPageForm ? "" : "w-full max-w-6xl"}>
            <div className={isFullPageForm ? "" : "rounded-lg border border-slate-200 bg-white shadow-sm"}>
              {!isFullPageForm ? (
              ...
              <div className={isFullPageForm ? "" : "p-4 md:p-6"}>{children}</div>
            </div>
          </div>
        </main>
      </div>
