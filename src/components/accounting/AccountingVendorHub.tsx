...
const SMART = [
  {
    ...
    label: "Ledger",
    icon: BookOpen,
    href: (id: string) => `/accounting/vendors/${id}/ledger`,
  },
  {
    id: "statement",
    label: "Statement",
    icon: ScrollText,
    href: (id: string) => `/accounting/vendors/${id}/statement`,
  },
  {
    id: "outstanding",
    label: "Outstanding",
    icon: FileText,
    href: (id: string) => `/accounting/vendors/${id}/bills?filter=outstanding`,
  },
  {
    id: "paid",
    label: "Paid",
    icon: CheckCircle2,
    href: (id: string) => `/accounting/vendors/${id}/bills?filter=paid`,
  },
  {
    id: "overdue",
    label: "Overdue",
    icon: AlertTriangle,
    href: (id: string) => `/accounting/vendors/${id}/bills?filter=overdue`,
  },
  {
    id: "timeline",
    label: "Timeline",
    icon: Clock,
    href: (id: string) => `/accounting/vendors/${id}/timeline`,
  },
  {
    id: "transactions",
    label: "Transactions",
    icon: History,
    href: (id: string) => `/accounting/vendors/${id}/transactions`,
  },
] as const;

export function AccountingVendorHub({ contactId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    void getVendorAccountingBalance(contactId).then((res) => {
      if (cancelled) return;
      if ("error" in res && res.error) setBalance(null);
      else if (res.balance) setBalance(res.balance);
    });
    return () => {
      cancelled = true;
    };
    ...
    <div className="space-y-3">
      {balance ? (
        <div className="grid gap-2 sm:grid-cols-4 bg-white border border-slate-200 rounded-sm p-3">
          ...
          <div>
            <p className="text-[11px] uppercase text-secondary-muted">Paid</p>
