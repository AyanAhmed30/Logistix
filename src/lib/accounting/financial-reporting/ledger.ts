...
export async function loadChartAccounts(
  ...
  let q = supabase
    ...
    .select('id, code, name, type, account_type, parent_id, organization_id, is_active')
    .neq('type', 'view')
    .eq('is_active', true)
...
export async function loadPostedLedgerFacts(
  ...
  if (query.organizationId) {
    q = q.eq('accounting_journal_entries.organization_id', query.organizationId);
  }
  if (query.dateFrom) {
    q = q.gte('accounting_journal_entries.entry_date', query.dateFrom);
  }
  if (query.dateTo) {
    q = q.lte('accounting_journal_entries.entry_date', query.dateTo);
  }
  if (query.accountIds?.length) {
    q = q.in('account_id', query.accountIds);
  }
  if (query.contactId) {
    q = q.or(
  ...
  const { data, error } = await q;
  if (error) {
    return loadPostedLedgerFactsTwoStep(query);
  }
  ...
  const journalIds = [
    ...new Set(
      (data || [])
        .map((row) => {
          const entryRaw = (row as { accounting_journal_entries?: unknown })
  ...
  for (const row of data || []) {
    ...
    facts.push(
      mapFact(row as Record<string, unknown>, e, jid ? journals.get(jid) : null)
    );
  }
  return facts;
}
...
async function loadPostedLedgerFactsTwoStep(
  ...
  for (let i = 0; i < ids.length; i += 200) {
    ...
    let lq = supabase
      .from('accounting_journal_entry_lines')
      .select(
        'id, journal_entry_id, account_id, debit, credit, label, partner_name, contact_id, amount_residual, is_reconciled'
      )
    ...
    if (lErr) {
      // Older schema without residual columns
      ...
      for (const line of fallback.data || []) {
        ...
        const j = e.journal_id ? journals.get(e.journal_id) : null;
        facts.push(
          mapFact(line as Record<string, unknown>, e as Record<string, unknown>, j)
        );
      }
      continue;
    }
