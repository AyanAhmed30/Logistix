...
/**
 * Accounting Customers — same Contacts rows (customer_rank > 0).
 * Server-side search + pagination for fast lists.
 */
export async function getAccountingCustomers(opts?: {
  search?: string;
  page?: number;
  pageSize?: number;
}) {
  try {
    ...
    if (scope && 'error' in scope) {
      if (scope.status === 403) {
        return { customers: [] as ContactWithRelations[], total: 0, page: 1, pageSize: 40 };
      }
    ...
    let query = supabase
      .from('contacts')
      .select(
        'id, name, company_name, company_type, email, phone, mobile, city, country, lead_id_formatted, organization_id, customer_rank, parent_id',
        { count: 'exact' }
      )
      .gt('customer_rank', 0)
      .is('parent_id', null);
